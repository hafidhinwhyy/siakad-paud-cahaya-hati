...
if(isset($_GET['action']) && $_GET['action'] == "hapus") {
    $id_pendaftar = $_GET['username'];
    $query_pendaftar = mysqli_query($conn, "SELECT * FROM pendaftar where username = '$id_pendaftar'");

    if(mysqli_num_rows($query_pendaftar)) {

        $data_pendaftar = mysqli_fetch_array($query_pendaftar);
        $id_user = $data_pendaftar['username'];


        //hapus foto pendaftar
        unlink('../uploads/siswa/'.$data_pendaftar['foto']);
        $sql_hapus_pendaftar = mysqli_query($conn, "DELETE FROM pendaftar where username = '$id_pendaftar'");

        //hapus user
        $sql_hapus_user = mysqli_query($conn, "DELETE FROM user where username = '$id_pendaftar'");
